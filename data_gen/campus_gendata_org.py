...
class Feeder_campus(Dataset):
    ...
    def load_datas(self):
        ...
        for i, data_path in enumerate(self.data_path):
            if self.inter:#区分单人和多人
                self.load_data(data_path, self.label_path[i])#多人
            else:
                self.load_data2(data_path, self.label_path[i])#单人
    ...
    def load_data2(self, data_path, label_path):
            ...
            for i, row in enumerate(reader):
                ...
                for j, r in enumerate(row):
                    if r:
                        ...
                        length = len(rrr)
                        xyz = [rrr[0:length//3], rrr[length//3:length//3*2], rrr[length//3*2:]]#//:取整除 #取x，y，z坐标
                        tmp[0, :, :] = xyz
                        tmps.append(tmp)
                    else:
                        pass
        ...
        for img_id in sample_seq:
            ...
            current_label = ""
            for i, p in enumerate(relation):#p是{person,interaction}
                if i==0:#i=0是交互部分，直接跳过
                    continue
                else:
                    ...
                    for c in range(self.C):
                        sample[c, 0, :, 0] = tmps[t][0, c, :]#把tmps的joint一一对应到sample
...
def gendata(data_path, label_path,
    ...
    print("\nfinal: ", len(feeder.sample_id), len(feeder.samples), len(feeder.label))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        #'--out_folder', default='./data/campus_train')
    ...
    for p in part:
        ...
        if arg.inter:
            ...
            gendata2(data_path, label_path, data_out_path, label_out_path)
