...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/campus/test_bone.yaml',
...
class Processor():
    ...
    def caculate(self, output, label, lens, idx, epoch):
        pre = torch.max(output.data, 1)[1]
        output = output.data.cpu().numpy()
        label = label.data.cpu().numpy()
        pre = pre.cpu().numpy()
        a = min(0.50 + epoch/100,1)
        part = lens // 10
        ...
        if part == 0:
            if idx != 0:
                for i in range(len(label)):
                    append_output[i][pre[i]], append_output[i][label[i]] = append_output[i][label[i]], append_output[i][pre[i]]
        elif (idx % part)/part < a:
            for i in range(len(label)):
                append_output[i][pre[i]], append_output[i][label[i]] = append_output[i][label[i]], append_output[i][pre[i]]
        return append_output
    ...
    def eval(self, epoch, save_score=True, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            data_len = len(self.data_loader[ln])
            #gc.collect()
            #torch.cuda.empty_cache()
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    append_output = self.caculate(output, label, data_len, batch_idx, epoch)
                    score_frag.append(append_output)
            ...
            self.print_log('\tAccuracy: {},  model: {}'.format(accuracy, self.arg.model_saved_name))
            if self.arg.phase == 'train':
            ...
            #print(len(self.data_loader[ln].dataset.sample_name), len(score))
            #print(self.data_loader[ln].dataset.sample_name)
            #print(score_dict)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.eval(epoch=10, save_score=self.arg.save_score, loader_name=['test'], wrong_file=wf, result_file=rf)
            self.print_log('Done.\n')
